...
fgh <- read_json('../steamerHROS.json',simplifyVector = TRUE)
...
fgp <- read_json('../steamerPROS.json',simplifyVector = TRUE)
...
fgh <- select(hplist,playerid,Player=Name,MLB=Org)
pplist <- getFGProspects(pos = "pit") %>% rename(playerid = PlayerId)
fgp <- select(pplist,playerid,Player=Name,MLB=Org)
...
# Never downgrade a real FG numeric id to an sa-prefix (CBS/minor-master) id.
# Prospects file lists MLB players who still have sa-ids; without this filter
# their real FG id in mymaster gets overwritten and joins on playerid break.
...
remove any duplicates, or empty playerids
```{r}
...
# Attach MLBAM id for Baseball Savant links downstream. Numeric FanGraphs
# playerids map via the Chadwick register; "sa"/"cbs" synthetic ids stay NA.
mlbxw <- fgToMlbCrosswalk()
mymaster$mlb_id <- mlbxw$mlb_id[match(as.character(mymaster$playerid), mlbxw$fg_id)]
...
Update position eligibility from CBS PosElig file
Uses the first position when a player has multiple positions
Should run this once and a while but not every time
Last run - 2/6/2026
```{r}
# pedf <- read.cbs(str_c("../",year,"PosElig.csv"))
# pedf <- dplyr::rename(pedf,posEl=Eligible) %>% select(playerid,posEl)
# # Extract first position (before the comma)
# pedf$npos <- str_split_i(pedf$posEl, ",", 1)
# pedf <- select(pedf, playerid, npos)
# 
# # Update mymaster with new positions
# mymaster <- left_join(mymaster, pedf, by = c('playerid'))
# mymaster <- mymaster %>% mutate(Pos = if_else(!is.na(npos), npos, Pos))
# mymaster <- select(mymaster, -npos)
# 
# write.csv(mymaster, "../mymaster.csv")
# print(str_c("Updated positions from CBS PosElig file: ", sum(!is.na(pedf$npos)), " players"))
```
