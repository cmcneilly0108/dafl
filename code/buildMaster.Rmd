...
Bugs to Fix
   - Luis Moreno - there are two that both play for the Mets, some matching on name only breaks it
   - Prospects that get called up - is fixed by manually downloading new prospect files from FG
      - This was introduced by the daily scrape no longer working
...
fgraphsp <- fgraphsp %>% filter(playerid != "")
...
nonamecbs <- nonamecbs %>% filter(Player != "")
...
07/07/25 - 70 prospects don't have any FG ids.  I'm deleting them for now.
