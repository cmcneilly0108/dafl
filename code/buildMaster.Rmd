...
fgh <- read_json('../steamerH2026.json',simplifyVector = TRUE)
...
fgp <- read_json('../steamerP2026.json',simplifyVector = TRUE)
...
# FG promotes minor leaguers from saXXXX -> numeric ids but never the reverse;
# block numeric -> sa replacements to stop ~20 prospects oscillating each run.
repl <- repl %>% filter(!(str_starts(newid, 'sa') & !str_starts(playerid, 'sa')))
